...
- **PURPOSE:** Stores official issued documents, PDF file paths, and cryptographic QR verification tokens.
- **COLUMNS:**
  - `surat_final_id` INT AUTO_INCREMENT
  - `pengajuan_id` INT NOT NULL
  - `nomor_surat_resmi` VARCHAR(100) NOT NULL
  - `file_pdf_path` VARCHAR(255) NOT NULL
  - `qr_verification_token` VARCHAR(128) NOT NULL
...
- **INDEXES:** `idx_final_qr` (`qr_verification_token` UNIQUE), `idx_final_nomor` (`nomor_surat_resmi` UNIQUE).

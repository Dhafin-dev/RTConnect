...
### REQ-010
- **Title:** Digital Signature Verification (QR Code Embedding)
- **Description:** For applications requesting digital signatures, the RT Head confirms authorization to embed a cryptographic QR verification token into the official PDF.
- **Actor:** RT Head (*Ketua RT*)
...
- **Expected Behavior:** System verifies PIN, generates a unique QR verification token, binds it to the generated PDF document, marks status as `Selesai`, and notifies the resident that the document is ready for download.
- **Priority:** MUST

...
| **RULE-021** | **Official GitHub Remote Repository Binding:** The project's official remote repository is bound to `https://github.com/Dhafin-dev/RTConnect.git` under remote alias `origin`. The authoritative primary branch is `main`. | STRICT |
| **RULE-022** | **Automated Git Lifecycle (Commit & Push):** Upon completing any task or file modification, the AI agent must proactively stage changes (`git add .`), compose a standardized Conventional Commit message (`feat:`, `fix:`, `docs:`, `style:`, `refactor:`, `test:`), and push to `origin main` (performing `git pull --rebase origin main` when required). | STRICT |
| **RULE-023** | **Credential & Secret Protection:** Never commit `.env` files, production API tokens, or unmasked secrets to the GitHub repository. All sensitive credentials must remain in `.gitignore`. | STRICT |
| **RULE-024** | **Obsidian Knowledge Base Synchronization:** Any structural changes to specifications or documentation must be mirrored into the dedicated Obsidian vault `RTConnect-KnowledgeBase`. | STRICT |
...
### Protocol C: GitHub Synchronization & Remote Verification
1. Target Remote URL: `https://github.com/Dhafin-dev/RTConnect.git`
2. Default Target Branch: `main`
3. Commit Message Format: `<type>(<scope>): <short imperative description>`
   - Examples: `docs(ai-rules): add github repository binding and sync protocols`
4. Post-Commit Action: Execute push to ensure changes are persisted remotely and accessible to all collaborators.
